...
function initMap() {
    ...
    document.getElementById("bike").innerHTML = "<b>Number of bicycle racks nearby</b>: ";
                        ...
                        $.each(results, function(key, value) {
                            ...
                            google.maps.event.addListener(markers[key], 'click', function() {
                                ...
                                for (var i = 0; i < washedData.length; i++) {
                                    var x3 = new google.maps.LatLng(washedData[i].lat, washedData[i].lng);
                                    //nearbyHouse.push(google.maps.geometry.spherical.computeDistanceBetween(x1, x3)/1000);
                                    washedData[i].nearby=google.maps.geometry.spherical.computeDistanceBetween(x1, x3)/1000;
                                }

                                for (var i = 0; i < washedData.length-1; i++) {
                                    for (var j = 0; j < washedData.length-1; j++) {
                                         if(washedData[j].nearby > washedData[j+1].nearby){
                                           var aux= washedData[j].nearby;
                                           washedData[j].nearby=washedData[j+1].nearby;
                                           washedData[j+1].nearby=aux;
                                         }
                                  }
                                }
                                ...
                                //document.getElementById("ok").innerHTML= washedData[0].nearby;
